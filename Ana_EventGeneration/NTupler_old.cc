...
//      TruthRaw_TJet_m2    .push_back(tempJet_TJet_m2);
        ...
        */
        ...
        for (unsigned int iJet = 0; iJet < inclusiveJetsTruthRaw.size(); iJet++) {
            ...
            TSub  T1SubOutputTrim  = T_1Subjet(groomedJet, 0.1, 0.6, 20);
            tempJet_T1jet_angle    = T1SubOutputTrim.min_angle;
            tempJet_T1jet          = T1SubOutputTrim.volatility;
        
            TSub  T2SubOutputTrim  = T_2Subjet(groomedJet, 0.1, 0.6, 20);
...
double T_reclustering(fastjet::PseudoJet& input, int algorithm, double minRadius, double maxRadius, int numRadii) {
    ...
    for (double r = minRadius; r <= maxRadius; r += deltaR) {
        fastjet::JetDefinition TjetDefinition(algorithm, r);
...
TSub T_1Subjet(fastjet::PseudoJet& input, double minRadius, double maxRadius, int numRadii) {
    std::vector<double> telescopingMass;
    double beta = 1.0;
    double mass = 0;
    ...
    tauAxis1.SetPxPyPzE(tau1axes[0].px(), tau1axes[0].py(), tau1axes[0].pz(), tau1axes[0].e());
    double minTauAxisAngle = tauAxis1.DeltaR(tauAxis1);
    ...
    for (double r = minRadius; r <= maxRadius; r += deltaR) {
        ...
        for (unsigned int c = 0; c < input.constituents().size(); c++) {
            ...
            double distanceToTauAxis1 = particle.DeltaR(tauAxis1);
            if (distanceToTauAxis1 <= R) Tsubjet1 += particle;
        }
        mass = Tsubjet1.M();
        if (mass > M0) telescopingMass.push_back(mass);
    }
    ...
    if (telescopingMass.size()>0) {
        result.volatility = getVolatility(telescopingMass);
    }
    else {
        std::cout << "WARNING: zero entries for T_1Subjet " << minRadius << " " << maxRadius << " " << numRadii << " " <<  std::endl;
        result.volatility=-1;
    }
    return result;
}
...
TSub T_2Subjet(fastjet::PseudoJet& input, double minRadius, double maxRadius, int numRadii) {
    ...
    tauAxis1.SetPxPyPzE(tau2axes[0].px(),tau2axes[0].py(),tau2axes[0].pz(),tau2axes[0].e());
    tauAxis2.SetPxPyPzE(tau2axes[1].px(),tau2axes[1].py(),tau2axes[1].pz(),tau2axes[1].e());
    double minTauAxisAngle = tauAxis1.DeltaR(tauAxis2);

    double distanceToTauAxis1,d2;
    double deltaR = (maxRadius - minRadius)/(numRadii-1);
    double R = minRadius;
    ...
    for (double r = minRadius; r <= maxRadius; r += deltaR) {
        ...
        for (unsigned int c = 0; c < input.constituents().size(); c++) {
            particle.SetPxPyPzE(input.constituents()[c].px(),input.constituents()[c].py(),input.constituents()[c].pz(),input.constituents()[c].e());
            distanceToTauAxis1 = particle.DeltaR(tauAxis1);
            d2 = particle.DeltaR(tauAxis2);
            if (distanceToTauAxis1 <= R && distanceToTauAxis1 < d2 ) {
                Tsubjet1 = Tsubjet1 + particle;
            }
            else if (d2 <= R && d2 < distanceToTauAxis1 ) {
                Tsubjet2 = Tsubjet2 + particle;
            }
        }
        mass = (Tsubjet1 + Tsubjet2).M();
        if (mass > M0) {m12s.push_back(mass);}
        R += deltaR;
    }
    TSub result;
    result.min_angle = minTauAxisAngle;
    if (m12s.size()>0) {
        ...
        std::cout << "WARNING: zero entries for T_2Subjet " << minRadius << " " << maxRadius << " " << numRadii << " " <<  std::endl;
        result.volatility=-1;
    }
    return result;
}
...
T3Sub T_3Subjet(fastjet::PseudoJet& input, double minRadius, double maxRadius, int numRadii) {
    ...
    tauAxis1.SetPxPyPzE(tau3axes[0].px(),tau3axes[0].py(),tau3axes[0].pz(),tau3axes[0].e());
    tauAxis2.SetPxPyPzE(tau3axes[1].px(),tau3axes[1].py(),tau3axes[1].pz(),tau3axes[1].e());
    tauAxis3.SetPxPyPzE(tau3axes[2].px(),tau3axes[2].py(),tau3axes[2].pz(),tau3axes[2].e());
    double tau3_DISTANCETOTAUAXIS12 = tauAxis1.DeltaR(tauAxis2);
    double tau3_DISTANCETOTAUAXIS13 = tauAxis1.DeltaR(tauAxis3);
    double tau3_D23 = tauAxis2.DeltaR(tauAxis3);
    double minTauAxisAngle = tau3_DISTANCETOTAUAXIS12;
    double D_mid = tau3_DISTANCETOTAUAXIS13;
    ...
    double distanceToTauAxis1, d2, d3;
    double deltaR = (maxRadius - minRadius)/(numRadii-1);
    double R      = minRadius;
    ...
    for (double r = minRadius; r <= maxRadius; r += deltaR) {
        ...
        for (unsigned int c = 0; c < input.constituents().size(); c++) {
            ...
            distanceToTauAxis1 = particle.DeltaR(tauAxis1);
            d2 = particle.DeltaR(tauAxis2);
            d3 = particle.DeltaR(tauAxis3);
            if (distanceToTauAxis1 <= R && distanceToTauAxis1 < d2 && distanceToTauAxis1 < d3) {
                Tsubjet1 = Tsubjet1 + particle;
            }
            else if (d2 <= R && d2 < distanceToTauAxis1 && d2 < d3) {
                Tsubjet2 = Tsubjet2 + particle;
            }
            else if (d3 <= R && d3 < distanceToTauAxis1 && d3 < d2) {
...
TSub T_4Subjet(fastjet::PseudoJet& input, double minRadius, double maxRadius, int numRadii) {
    ...
    tauAxis1.SetPxPyPzE(tau4axes[0].px(),tau4axes[0].py(),tau4axes[0].pz(),tau4axes[0].e());
    tauAxis2.SetPxPyPzE(tau4axes[1].px(),tau4axes[1].py(),tau4axes[1].pz(),tau4axes[1].e());
    tauAxis3.SetPxPyPzE(tau4axes[2].px(),tau4axes[2].py(),tau4axes[2].pz(),tau4axes[2].e());
    tauAxis4.SetPxPyPzE(tau4axes[3].px(),tau4axes[3].py(),tau4axes[3].pz(),tau4axes[3].e());
    double tau4_DISTANCETOTAUAXIS12 = tauAxis1.DeltaR(tauAxis2);
    double tau4_DISTANCETOTAUAXIS13 = tauAxis1.DeltaR(tauAxis3);
    double tau4_DISTANCETOTAUAXIS14 = tauAxis1.DeltaR(tauAxis4);
    double tau4_D23 = tauAxis2.DeltaR(tauAxis3);
    double tau4_D24 = tauAxis2.DeltaR(tauAxis4);
    double tau4_D34 = tauAxis3.DeltaR(tauAxis4);
    double minTauAxisAngle = tau4_DISTANCETOTAUAXIS12;
    if (tau4_DISTANCETOTAUAXIS13 < minTauAxisAngle ) minTauAxisAngle = tau4_DISTANCETOTAUAXIS13;
    if (tau4_DISTANCETOTAUAXIS14 < minTauAxisAngle ) minTauAxisAngle = tau4_DISTANCETOTAUAXIS14;
    if (tau4_D23 < minTauAxisAngle ) minTauAxisAngle = tau4_D23;
    if (tau4_D24 < minTauAxisAngle ) minTauAxisAngle = tau4_D24;
    if (tau4_D34 < minTauAxisAngle ) minTauAxisAngle = tau4_D34;

    double distanceToTauAxis1, d2, d3, d4;
    double deltaR = (maxRadius - minRadius)/(numRadii-1);
    double R      = minRadius;
    ...
    for (double r = minRadius; r <= maxRadius; r += deltaR) {
        ...
        for (unsigned int c = 0; c < input.constituents().size(); c++) {
            ...
            d2 = particle.DeltaR(tauAxis2);
            d3 = particle.DeltaR(tauAxis3);
            d4 = particle.DeltaR(tauAxis4);
            if (distanceToTauAxis1 < R && distanceToTauAxis1 < d2 && distanceToTauAxis1 < d3 && distanceToTauAxis1 < d4) {
                Tsubjet1 = Tsubjet1 + particle;
            }
            else if (d2 < R && d2 < distanceToTauAxis1 && d2 < d3 && d2 < d4) {
                Tsubjet2 = Tsubjet2 + particle;
            }
            else if (d3 < R && d3 < distanceToTauAxis1 && d3 < d2 && d3 < d4) {
                Tsubjet3 = Tsubjet3 + particle;
            }
            else if (d4 < R && d4 < distanceToTauAxis1 && d4 < d2 && d4 < d3) {
                Tsubjet4 = Tsubjet4 + particle;
            }
        }
        mass = (Tsubjet1 + Tsubjet2 + Tsubjet3 + Tsubjet4).M();
        if (mass > M0) {m1234s.push_back(mass);}
        R += deltaR;
    }
    TSub result;
    result.min_angle = minTauAxisAngle;
    if (m1234s.size()>0) {
        ...
        std::cout << "WARNING: zero entries for T_4Subjet " << minRadius << " " << maxRadius << " " << numRadii << " " <<  std::endl;
        result.volatility = -1;
    }
    return result;
}

TSub T_5Subjet(fastjet::PseudoJet& input, double minRadius, double maxRadius, int numRadii) {
    ...
    tauAxis1.SetPxPyPzE(tau5axes[0].px(),tau5axes[0].py(),tau5axes[0].pz(),tau5axes[0].e());
    tauAxis2.SetPxPyPzE(tau5axes[1].px(),tau5axes[1].py(),tau5axes[1].pz(),tau5axes[1].e());
    tauAxis3.SetPxPyPzE(tau5axes[2].px(),tau5axes[2].py(),tau5axes[2].pz(),tau5axes[2].e());
    tauAxis4.SetPxPyPzE(tau5axes[3].px(),tau5axes[3].py(),tau5axes[3].pz(),tau5axes[3].e());
    tauAxis5.SetPxPyPzE(tau5axes[4].px(),tau5axes[4].py(),tau5axes[4].pz(),tau5axes[4].e());
    double tau5_DISTANCETOTAUAXIS12 = tauAxis1.DeltaR(tauAxis2);
    double tau5_DISTANCETOTAUAXIS13 = tauAxis1.DeltaR(tauAxis3);
    double tau5_DISTANCETOTAUAXIS14 = tauAxis1.DeltaR(tauAxis4);
    double tau5_DISTANCETOTAUAXIS15 = tauAxis1.DeltaR(tauAxis5);
    double tau5_D23 = tauAxis2.DeltaR(tauAxis3);
    double tau5_D24 = tauAxis2.DeltaR(tauAxis4);
    double tau5_D25 = tauAxis2.DeltaR(tauAxis5);
    double tau5_D34 = tauAxis3.DeltaR(tauAxis4);
    double tau5_D35 = tauAxis3.DeltaR(tauAxis5);
    double tau5_D45 = tauAxis4.DeltaR(tauAxis5);
    double minTauAxisAngle = tau5_DISTANCETOTAUAXIS12;
    if (tau5_DISTANCETOTAUAXIS13 < minTauAxisAngle ) minTauAxisAngle = tau5_DISTANCETOTAUAXIS13;
    if (tau5_DISTANCETOTAUAXIS14 < minTauAxisAngle ) minTauAxisAngle = tau5_DISTANCETOTAUAXIS14;
    if (tau5_DISTANCETOTAUAXIS15 < minTauAxisAngle ) minTauAxisAngle = tau5_DISTANCETOTAUAXIS15;
    if (tau5_D23 < minTauAxisAngle ) minTauAxisAngle = tau5_D23;
    if (tau5_D24 < minTauAxisAngle ) minTauAxisAngle = tau5_D24;
    if (tau5_D25 < minTauAxisAngle ) minTauAxisAngle = tau5_D25;
    if (tau5_D34 < minTauAxisAngle ) minTauAxisAngle = tau5_D34;
    if (tau5_D35 < minTauAxisAngle ) minTauAxisAngle = tau5_D35;
    if (tau5_D45 < minTauAxisAngle ) minTauAxisAngle = tau5_D45;
    
    double distanceToTauAxis1, d2, d3, d4, d5;
    double deltaR = (maxRadius - minRadius)/(numRadii-1);
    double R      = minRadius;
    ...
    for (double r = minRadius; r <= maxRadius; r += deltaR) {
        ...
        for (unsigned int c = 0; c < input.constituents().size(); c++) {
            ...
            d3 = particle.DeltaR(tauAxis3);
            d4 = particle.DeltaR(tauAxis4);
            d5 = particle.DeltaR(tauAxis5);
            if (distanceToTauAxis1 < R && distanceToTauAxis1 < d2 && distanceToTauAxis1 < d3 && distanceToTauAxis1 < d4 && distanceToTauAxis1 < d5) {
                Tsubjet1 = Tsubjet1 + particle;
            }
            else if (d2 < R && d2 < distanceToTauAxis1 && d2 < d3 && d2 < d4 && d2 < d5) {
                Tsubjet2 = Tsubjet2 + particle;
            }
            else if (d3 < R && d3 < distanceToTauAxis1 && d3 < d2 && d3 < d4 && d3 < d5) {
                Tsubjet3 = Tsubjet3 + particle;
            }
            else if (d4 < R && d4 < distanceToTauAxis1 && d4 < d2 && d4 < d3 && d4 < d5) {
                Tsubjet4 = Tsubjet4 + particle;
            }
            else if (d5 < R && d5 < distanceToTauAxis1 && d5 < d2 && d5 < d3 && d5 < d4) {
                Tsubjet5 = Tsubjet5 + particle;
            }
        }
        mass = (Tsubjet1 + Tsubjet2 + Tsubjet3 + Tsubjet4 + Tsubjet5).M();
        if (mass > M0) {m12345s.push_back(mass);}
        R += deltaR;
    }
    TSub result;
    result.min_angle = minTauAxisAngle;
