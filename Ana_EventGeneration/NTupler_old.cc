...
vector<fastjet::PseudoJet> input_particles_Pileup;
...
std::vector<fastjet::PseudoJet> calo_cells_Pileup = ToyCalorimeter(input_particles_Pileup);
	...
	std::vector<fastjet::PseudoJet> inclusive_jets_TruthRaw = sorted_by_pt(clust_seq_TruthRaw.inclusive_jets(5.0));
        ...
        */
        ...
        double Rfilt0 = 0.3;
        double fcut0 = 0.05;
	fastjet::Transformer *trimmer = new Filter(JetDefinition(kt_algorithm, Rfilt0), SelectorPtFractionMin(fcut0) );
...
TSub T_1Subjet(fastjet::PseudoJet& input, double R_min, double R_max, int N_R) {
    ...
    for (int i = 0; i < N_R; i++) {
        ...
        for (unsigned int c = 0; c < input.constituents().size(); c++) {
            ...
            if (d1 <= R) Tsubjet1 = Tsubjet1 + particle;
        }
        Tmass = Tsubjet1.M();
        if (Tmass > M0) {ms.push_back(Tmass);}
        R += deltaR;
...
T3Sub T_3Subjet(fastjet::PseudoJet& input, double R_min, double R_max, int N_R) {
    ...
    for (int i = 0; i < N_R; i++) {
	    ...
	    double minDeltaR = std::min({d1, d2, d3});
	    
            if (d1 <= R && d1 == minDeltaR) {
		Tsubjet1 = Tsubjet1 + particle;
            }
	    else if (d2 <= R && d2 == minDeltaR) {
                Tsubjet2 = Tsubjet2 + particle;
            }
            else if (d3 <= R && d3 == minDeltaR) {
	...
	double wNeighborhood12 = std::abs(Tmass_12 - mW);
	double wNeighborhood13 = std::abs(Tmass_13 - mW);
	double wNeighborhood23 = std::abs(Tmass_23 - mW);
	double minWNeighborhood = std::min({wNeighborhood12, wNeighborhood13, wNeighborhood23});
        if (wNeighborhood12 == minWNeighborhood) {
            result.mass_W = Tmass_12;
            result.volatility_mass_W = getVolatility(m12s);
        }
        else if (wNeighborhood13 == minWNeighborhood) {
            result.mass_W = Tmass_13;
            result.volatility_mass_W = getVolatility(m13s);
        }
        else if (wNeighborhood23 == minWNeighborhood) {
...
TSub T_4Subjet(fastjet::PseudoJet& input, double R_min, double R_max, int N_R) {
    ...
    for (int i = 0; i < N_R; i++) {
	    ...
	    double minDeltaR = std::min({d1, d2, d3, d4});
	    if (d1 < R && d1 == minDeltaR) {
                Tsubjet1 = Tsubjet1 + particle;
            }
            else if (d2 < R && d2 == minDeltaR) {
...
TSub T_5Subjet(fastjet::PseudoJet& input, double R_min, double R_max, int N_R) {
    ...
    for (int i = 0; i < N_R; i++) {
	    ...
	    double minDeltaR = std::min({d1, d2, d3, d4, d5});
            if (d1 < R && d1 == minDeltaR) {
                Tsubjet1 = Tsubjet1 + particle;
            }
            else if (d2 < R && d2 == minDeltaR) {
