...
def OverlayThreePanelROCS(outputdir1,outputdir2,outputdir3,outputdir4,mvatypes,VarsAndRanges):
    ...
    for CutRegion in CutRegions:
        ...
        f7   = TFile(path)
        roc7 = f7.Get("ROC_L")
        roc7.SetLineColor(4) #95
        roc7.SetLineStyle(1)

        path = outputdir3+"W_TMVAOutput__"+algs[1]+"__BDTAllTjet__pt"+pt1+pt2+"_ROCSBDT.root"
        f8   = TFile(path)
        roc8 = f8.Get("ROC_L")
        roc8.SetLineColor(4) #95
        roc8.SetLineStyle(2)
        ...
        f11   = TFile(path)
        roc11 = f11.Get("ROC_L")
        roc11.SetLineColor(8)
        roc11.SetLineStyle(1)
        
        path = outputdir1+"W_ROC_CaloRaw_Ttrimming_pt"+pt1+pt2+".root"
        f12   = TFile(path)
        roc12 = f12.Get("ROC_L")
        roc12.SetLineColor(8)
        roc12.SetLineStyle(2)
        ...
        path = outputdir1+"W_ROC_"+algs[1]+"_T2jet_pt"+pt1+pt2+".root"
        #path = outputdir1+"W_ROC_"+algs[1]+"_v21_pt"+pt1+pt2+".root"
        f14   = TFile(path)
        roc14 = f14.Get("ROC_L")
        roc14.SetLineColor(95)
        roc14.SetLineStyle(2)
        ...
        #myText(       0.20,0.83,1,0.04, "#font[132]{#sqrt{s} = 13 TeV}, |#eta|<1.2")
        rocbox5=myLineBoxText(0.26,  0.68, 1, 1, 1, 0, 0.1, 0.08, "#font[132]{"+TranslateVar("Tau21")+"}")
        rocbox3=myLineBoxText(0.26,  0.63, 2, 1, 1, 0, 0.1, 0.08, "#font[132]{"+TranslateVar("T3jet")+"}")
        rocbox13=myLineBoxText(0.26, 0.58, 95, 1, 1, 0, 0.1, 0.08, "#font[132]{"+TranslateVar("T2jet")+"}")
        #rocbox13=myLineBoxText(0.26, 0.58, 95, 1, 1, 0, 0.1, 0.08, "#font[132]{"+TranslateVar("v21")+"}")
        rocbox7=myLineBoxText(0.26,  0.53, 4, 1, 1, 0, 0.1, 0.08, "#font[132]{BDT(3)}")
        #rocbox9=myLineBoxText(0.26,  0.48, 8, 1, 1, 0, 0.1, 0.08, "#font[132]{"+TranslateVar("Tpruning")+"}")
        rocbox11=myLineBoxText(0.26, 0.48, 8, 1, 1, 0, 0.1, 0.08, "#font[132]{"+TranslateVar("Ttrimming")+"}")
    ...
    #myText(       0.20,0.83,1,0.04, "#font[132]{#sqrt{s} = 13 TeV}, |#eta|<1.2")
    #myText(       0.20,0.73,1,0.04, "#font[132]{"+m1+" < m < "+m2+" GeV}")
    rocbox5=myLineBoxText(0.26, 0.68, 1, 1, 1, 0, 0.1, 0.08,   "#font[132]{"+TranslateVar("Tau21")+" (truth)}")
    rocbox6=myLineBoxText(0.26, 0.63, 923, 2, 1, 0, 0.1, 0.08, "#font[132]{"+TranslateVar("Tau21")+" (cell)}")
    rocbox3=myLineBoxText(0.26, 0.58, 2, 1, 1, 0, 0.1, 0.08,  "#font[132]{"+TranslateVar("T3jet")+" (truth)}")
    rocbox4=myLineBoxText(0.26, 0.53, 95, 2, 1, 0, 0.1, 0.08,"#font[132]{"+TranslateVar("T3jet")+" (cell)}")
